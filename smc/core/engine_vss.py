...
class VSSContainer(MasterEngine):
    ...
    def container_node(self):
        return [VSSContainerNode(**node)
                for node in self.read_cmd(resource='vss_container_node')]

    @property
    def default_settings(self):
        return self.read_cmd(resource='default_settings')
    ...
    def isc_settings(self):
        ...
        """
        return self.read_cmd(resource='isc_settings')
    ...
    def vss_contexts(self):
        ...
        return [VSSContext(**context)
                for context in self.read_cmd(resource='vss_contexts')]

    @property
    def security_groups(self):
        return [SecurityGroup(**group)
                for group in self.read_cmd(resource='security_groups')]
