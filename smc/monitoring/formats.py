...
class FormatFieldMixin(object):
    def field_ids(self, ids):
        """
        Show only fields with given field ID's. Field ID's can be mapped
        to the LogField constants. For example, build a query that returns
        timestamp, node (fw) identifier, source, destination, action, and
        protocol, and also displays the timestamp field in CST format::
            ...
            query.format.set_resolving(timezone='CST')
    ...
    def field_names(self, names):
        ...
        For example, build a query that returns timestamp, node (fw)
        identifier, source, destination, action, and protocol, and also
        displays the timestamp field in CST format::
        
            query = LogQuery(fetch_size=5)
            query.format.field_names([
                'Timestamp', 'NodeId', 'Src', 'Dst', 'Protocol', 'Action'
                ])
            query.format.set_resolving(timezone='CST')
