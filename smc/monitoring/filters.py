"""
Filters are only used by Log queries to limit the search based on specified
criteria.
...
class InFilter(QueryFilter):
    ...
    :param Value left: single value for leftmost portion of filter
    :param list(Value) right: list of values for rightmost portion of filter    
    """
...
class TranslatedFilter(QueryFilter):
    ...
    def within_ipv4_network(self, field, values):
        """
        Filter within a subnet formatted range/s.
        
        :param str name of field to filter on. Taken from 'Show Filter
            Expression' within SMC.
        :param list values: network definitions, in format '1.1.1.0/24'.
        """
